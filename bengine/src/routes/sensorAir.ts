...
router.get('/sensor/air', async (req, res) => {
    try {
        ...
        const data = await Air.findAll(
            fiveMinuteAggregationByDevice(deviceId, startTime)
        )
        res.send(data)
    } catch (err) {
        console.error(err)
...
const fiveMinuteAggregationByDevice: (
    deviceId: string,
    startTime: string
) => FindOptions<any> = (deviceId: string, startTime: string) => ({
    attributes: [
        [Sequelize.literal("time_bucket('5 minutes', time)"), 'time_bucket'],
        [Sequelize.literal('ROUND(AVG(temperature))'), 'temperature'],
        [Sequelize.literal('ROUND(AVG(humidity))'), 'humidity'],
        [Sequelize.literal('ROUND(AVG(co2))'), 'co2'],
    ],
    where: {
        device_id: deviceId,
        time: {
